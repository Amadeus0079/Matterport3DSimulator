...
class Instruction_Analysis_Experts:
    def detect_actions(self, instruction):
        # instruction = gpt_response([{"role": "user", "content": f"Please translate the sentence \"{instruction}\" into English. Your translation is: "}], "gpt-4o-mini", 1)[0]["message"]["content"]
        prompt = [
        ...
        return gpt_response(prompt, "gpt-4o-mini", 1)[0]["message"]["content"]
...
class Completion_Estimation_Experts:
    def summarize_observation(self, curr_observe, landmarks):
        ...
        return f"Direction {direction} " + gpt_response(prompt, "gpt-4o-mini", 1)[0]["message"]["content"]
    ...
    def estimate_completion(self, actions, landmarks, history_traj):
        ...
        response = gpt_response(prompt, "gpt-4o-mini", 1)[0]["message"]["content"]
...
class DiscussNav_Agent:
    ...
    def pred_vp(self, current_step, instruction, actions, landmarks, history_traj, estimation, observation):
        ...
        for _ in range(self.num_retry):
            ...
            batch_responses = gpt_response(decision_prompt, "gpt-4o-mini", self.num_predictions)
...
class Decision_Testing_Experts:
    def thought_fusion(self, predictions, thoughts):
        ...
        for key, value in matched_dict.items():
            ...
            one_thought = gpt_response(prompt, "gpt-4o-mini", 1)[0]["message"]["content"]  
    ...
    def test_decisions(self, fused_pred_thought, observation, instruction):
        if len(fused_pred_thought.keys()) == 1:
            ...
            for _ in range(3):
                next_vp = gpt_response(decision_prompt, "gpt-4o-mini", 1, temperature=0)[0]["message"]["content"].strip()
                if len(next_vp) != 32:
...
def eval_pred(final_vp, nav_history, shortest_distances, eval_cache, instruction, scanId, des_vp, gt_path):
    ...
    trajectory_length = int(np.sum([shortest_distances[scanId][a][b] for a, b in zip(path[:-1], path[1:])]))
...
if __name__ == "__main__":
    dataset_name = "R2R"
    ram_path = "pretrained/ckpt/ram_swin_large_14m.pth"
